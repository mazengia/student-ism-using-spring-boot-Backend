...
public class FailedTrafficController implements FailedTrafficApi {
    ...
    @Override
    public ResponseEntity<PagedModel<FailedTrafficDtos>> getAllTraffics(Pageable pageable, PagedResourcesAssembler assembler, UriComponentsBuilder uriBuilder, HttpServletResponse response) {
        eventPublisher.publishEvent(new PaginatedResultsRetrievedEvent<>(
                FailedTrafficDtos.class, uriBuilder, response, pageable.getPageNumber(), failedTrafficService.getAllTraffics(pageable).getTotalPages(), pageable.getPageSize()));
        return new ResponseEntity<PagedModel<FailedTrafficDtos>>(assembler.toModel(failedTrafficService.getAllTraffics(pageable).map(failedTrafficMapper::toTrafficsDto)), HttpStatus.OK);
    }
}
