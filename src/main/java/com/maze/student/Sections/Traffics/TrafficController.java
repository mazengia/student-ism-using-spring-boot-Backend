...
public class TrafficController implements TrafficApi {
    ...
    @Override
    public ResponseEntity<PagedModel<TrafficDtos>> getAllTraffics(Pageable pageable, PagedResourcesAssembler assembler, UriComponentsBuilder uriBuilder, HttpServletResponse response) {
        eventPublisher.publishEvent(new PaginatedResultsRetrievedEvent<>(
                TrafficDtos.class, uriBuilder, response, pageable.getPageNumber(), trafficService.getAllTraffics(pageable).getTotalPages(), pageable.getPageSize()));
        return new ResponseEntity<PagedModel<TrafficDtos>>(assembler.toModel(trafficService.getAllTraffics(pageable).map(trafficMapper::toTrafficsDto)), HttpStatus.OK);
    }
}
